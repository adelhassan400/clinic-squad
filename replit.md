...
- **Email Verification**: New registrations create a user with `users.email_verified_at = null` and issue a 24-hour single-use verification token (stored hashed in `email_verification_tokens`). Because no email integration is configured, the token + verify URL are returned in the registration response and shown directly on the register page so users can copy them. `/verify-email?token=…` consumes the token, sets `email_verified_at`, and returns an authenticated session (so verification logs the user straight in). `/resend-verification` issues a fresh token by email (generic response when email is unknown or already verified — no enumeration). Login is blocked with `403 { code: "email_not_verified" }` until the email is verified — the login page surfaces a banner with a "Resend verification link" CTA. Endpoints: `POST /api/auth/verify-email`, `POST /api/auth/resend-verification`. Demo seed accounts (`admin@demo.com`, `super@clinicsquad.com`) are created pre-verified. Auth event type added: `email_verified`.
