...
- **Patient Profile & Workflow**: The patient detail page (`/patients/:id`) ships an enhanced workflow:
  - **Edit Profile**: Outlined "Edit Profile" button at the top of the profile card opens a dialog to update Name, Age, Phone, and Chronic Conditions. PATCH `/api/clinics/:clinicId/patients/:id` accepts `chronicConditions` (text, nullable, stored on `patients.chronic_conditions`).
  - **Chronic Conditions display**: Shown as an amber-tinted block on the profile card alongside Allergies and Medical Notes.
  - **Lab Results section** (own card under Doctor's Session): "Add Lab Result" reveals a form (Test Name, Date, Result Value, Attachment ≤ 5 MB), and lists existing results in a table with download + delete. Attachments are sent as base64 data URLs (no multer); Express body limit was bumped to 8 MB. New table `lab_results` (id, clinic_id, patient_id, test_name, test_date, result_value, attachment_name/mime/data, created_at). Endpoints: `GET/POST/DELETE /api/clinics/:clinicId/patients/:patientId/lab-results`.
  - **Sticky safety banner above prescriptions**: An amber alert ("Check before prescribing") appears sticky at the top of the embedded `<PrescriptionsContent>` whenever the patient has any of Allergies / Chronic Conditions / Medical Notes — it stays visible while the doctor scrolls through the prescription form.
  - **Finish & Next button**: Sits next to the existing "Finish Session" button. Marks the patient `completed`, then navigates to the next `waiting` patient (sorted by `createdAt` ascending) or back to `/waiting-list` when the queue is empty. The button label shows the next patient's name when one exists.
...
- **ePrescription** (available on all paid plans, admin-create): per-patient or global `/prescriptions` page (label "ePrescription"), multi-medication form with **live preview panel** (right side, mirrors the print layout — shows clinic name, doctor name + medical specialty, patient name + auto-generated patient code, date, diagnosis, medications added so far, notes, signature). Printable A4 layout (window.print → save as PDF) and "Send to patient WhatsApp" via `wa.me` deep link include the patient code and doctor specialty. Entry points: sidebar nav, patient detail page, and per-row button on the appointments list.
  - **Clinic Branding** (Apr 2026): Rx header centers the **clinic name** in brand teal `#20B8AD` with a horizontal underline; doctor name + specialty appear in a smaller strip beneath. Footer has a thin teal horizontal line then **Address** (location-pin icon) and **Phone** (phone icon) side-by-side at ~10pt. `@page` rule sets A4 with 12 mm/10 mm margins so print and PDF respect page edges. Both `printPrescription(p, branding)` and `sendPrescriptionWhatsApp(p, branding)` accept a `ClinicBranding = { name, phone, address }` object; the WhatsApp message also includes the address/phone footer. The on-screen LivePreview mirrors the print layout exactly.
- **Clinic Identity Settings** (Apr 2026): Settings page has a "Clinic Identity" card (admin/superadmin only) with editable **Clinic Name**, **Phone**, and **Address** (textarea). Persisted on `clinics` table via `phone TEXT` and `address TEXT` (nullable) columns. New endpoint `PATCH /api/clinics/:clinicId` (operationId `updateClinic`, body `UpdateClinicBody { name?, phone?, address? }`) updates these fields; clinic GET, login response, and email-verify response all serialize `phone` and `address`. The auth context's `updateClinic()` keeps the in-memory clinic object in sync after save so the prescription preview re-renders instantly.
...
- Visit Type system: `artifacts/clinic-squad/src/lib/visit-types.tsx` exports `VISIT_TYPES`, `VisitTypeBadge`, `getVisitTypeStyle`, and `DEFAULT_VISIT_TYPE_PRICES`. Used by appointments form (Select dropdown with auto-priced options), list view, day calendar (color bar), and dashboard today's appointments. Colors: New Consultation=blue, Follow-up=green, Re-exam=amber, Emergency=red, Procedure=purple. Free-form legacy values fall back to a neutral muted badge.
- Visit Type pricing: `artifacts/clinic-squad/src/lib/visit-prices.ts` provides `useVisitTypePrices(clinicId)` and `getVisitTypePrice` backed by `localStorage` (`clinicsquad:visit-prices:{clinicId}`). Configurable in Settings → "Visit Type Pricing" (admin-only). Selecting a visit type in the appointment form auto-fills the "Amount to Pay" field (still manually editable).
- Patient workflow (Apr 2026): `patients` table has `status` (`waiting`|`in-progress`|`completed`, default `waiting`), `diagnosis`, `clinicalNotes`. `gender` was removed. `visitType` is required (enum: New Consultation, Follow-up, Re-exam, Emergency). PATCH `/api/clinics/:clinicId/patients/:patientId` does partial updates. GET supports `?status=` filter. Pages: `/patients` (list cols Name/Age/Phone/Visit Type), `/waiting-list` (auto-listed waiting patients with colored visit-type badge **and queue position number per row**; in-progress shows • marker; queue numbering only counts waiting patients), `/patient-detail/:id` (auto-promotes to in-progress, debounced autosave for diagnosis & clinical notes via `usePatchPatient`, Finish Session → completed), `/checkout` (completed patients with Amount Due from visit-type pricing).
- Appointment check-in flow (Apr 2026): Appointment status enum extended to `[scheduled, checked_in, completed, cancelled, no_show]`. On the Appointments page (list view + day calendar), a **Check In** button appears on `scheduled` appointments dated today. It runs PUT `/appointments/:id` with `status: "checked_in"`, then PATCH `/patients/:id` with `status: "waiting", diagnosis: null, clinicalNotes: null` so the patient appears fresh on the Waiting List. Both list queries are then invalidated.
- Theme: primary = Professional teal #20B8AD (HSL 176 70% 42%) with darker hover #1A998F (HSL 176 71% 35%, exposed via `--primary-hover` CSS var and used by the default Button variant), accent = Lavender (HSL 265 60% 70%). Defined in `artifacts/clinic-squad/src/index.css`. The patient-initial avatar circle on the Dashboard "Tomorrow's Appointments" card uses `bg-primary/15` + `text-primary` to track the brand. Status colors elsewhere (text-green-600 for income, "completed" badges, "Follow-up" visit-type badge, WhatsApp green) are intentionally semantic and not part of the brand swap.
- Drizzle interactive prompts: `pnpm --filter @workspace/db run push` is interactive when distinguishing column rename vs create, and ignores piped stdin. For non-rename additions/drops, use `psql "$DATABASE_URL"` directly with idempotent `ALTER TABLE ... IF EXISTS / IF NOT EXISTS` statements.
