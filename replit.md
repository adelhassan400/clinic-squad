...
- **Monthly Revenue (superadmin)**: Admin panel includes a Monthly Revenue panel — current-month total (highlighted), last-3-months total with average (or per-year average when a specific year is selected), and best month — followed by a 12-month bar chart (recharts) and a tabular breakdown of every month with payment count and revenue. A year selector in the panel header switches between "Last 12 months" (rolling) and any specific year (Jan–Dec). Data is bucketed in UTC by `subscriptions.createdAt` for confirmed payments only. `GET /api/admin/stats` accepts an optional `?year=YYYY` query param and returns `revenueByMonth: { month, amount, count }[]`, `currentMonthRevenue` (always live, regardless of selected year), `revenueRange: { mode: "rolling12" } | { mode: "year", year }`, and `availableYears: number[]` (years with any confirmed payment plus the current year).
