...
- **Monthly Revenue (superadmin)**: Admin panel includes a Monthly Revenue panel — current-month total (highlighted), last-3-months total with average, and best month — followed by a 12-month bar chart (recharts) and a tabular breakdown of every month with payment count and revenue. Data is bucketed in UTC by `subscriptions.createdAt` for confirmed payments only, returned by the existing `GET /api/admin/stats` endpoint as `revenueByMonth: { month, amount, count }[]` plus `currentMonthRevenue`.
