...
UPDATE patients
SET payment_received_at = COALESCE(payment_received_at, updated_at)
WHERE status IN ('paid', 'waiting', 'in-progress', 'completed')
...
-- Existing rows are left otherwise untouched. New payments are written atomically by queue registration.
